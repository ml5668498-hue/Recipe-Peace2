...
import { Loader2, Leaf, Clock, X, ChefHat } from "lucide-react";
              ...
              {generateRecipes.data.recipes.map((recipe, index) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3 gap-4">
                    <h4 className="font-serif text-[19px] font-medium text-foreground leading-snug">{recipe.name}</h4>
                  </div>
                  
                  <div className="flex flex-wrap gap-2 mb-4">
                    <Badge variant="secondary" className="bg-secondary/30 text-secondary-foreground hover:bg-secondary/40 font-normal rounded-md px-2 py-0.5 border-none flex gap-1.5 items-center">
                  ...
                  <div className="mb-4">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Ingredientes usados</p>
                    <p className="text-sm text-foreground/80">{recipe.usedIngredients.join(", ")}</p>
                  </div>

                  <div className="mb-5">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Pasos sencillos</p>
                  ...
                  <div className="bg-primary/5 rounded-xl p-4 flex gap-3 items-start border border-primary/10">
                    ...
                    <p className="text-[14px] text-foreground/80 italic leading-relaxed">"{recipe.antiAnxietyTip}"</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
